...
static bool enable_bbd_wl = true;
...
static bool enable_ssp_comm_wl = true;
...
static bool enable_bt_wake_wl = true;
module_param(enable_bt_wake_wl, bool, 0644);

static bool enable_mmc_wl = true;
...

{
	...
	if (ws) {
		...
		if ((!enable_sensorhub_wl && !strncmp(ws->name, "ssp_sensorhub_wake_lock", wslen)) ||
			...
			(!enable_ssp_comm_wl &&
				!strncmp(ws->name, "ssp_comm_wake_lock", wslen))) {
			if (ws->active) {
